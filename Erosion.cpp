...
void Erosion::processSidechainedBlock(dsp::AudioBlock<float>& block, dsp::AudioBlock<float>& sidechain, double sampleRate) {
    auto leftBlock = block.getSingleChannelBlock(0);
    auto rightBlock = block.getSingleChannelBlock(1);
    ...
    // get the current value of the saturation knob to figure out how much to multiply our sidechain value by
    float saturationAmount = erosionAmount->get() * 0.5f;
    ...
    *iirFilter.state = *dsp::IIR::Coefficients<float>::makeBandPass(sampleRate, erosionFrequency->get(), erosionQ->get());
    ...
    //     float randomValue = sidechain.getSample(0, i) * 2.0f - 1.0f;
    //     randomBuffer.setSample(0, i, randomValue);

    //     randomValue = sidechain.getSample(1, i) * 2.0f - 1.0f;
    //     randomBuffer.setSample(1, i, randomValue);
    // }
    ...
    for (int i = 0; i < block.getNumSamples(); i++) {
        float leftEroded = delayLine.popSample(0, (sidechain.getSample(0, i) + 1.f) * saturationAmount);
        float rightEroded = delayLine.popSample(1, (sidechain.getSample(1, i) + 1.f) * saturationAmount);
